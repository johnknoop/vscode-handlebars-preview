...
async function getCompiledHtml(
	templateDocument: TextDocument,
	contextFile: string,
	panel: PreviewPanelScope
...
function repathLocalFiles(html: string, templateDocument: TextDocument) {
	...
	$('img')
		...
		.each((i, elm) => {
			// satisfy typing
			if (elm.type === 'tag') {
				elm.attribs['src'] = Uri.file(path.join(path.dirname(templateDocument.fileName), elm.attribs['src']))
	...
	$('link')
		...
		.each((i, elm) => {
			...
			if (elm.type === 'tag') {
				...
				const newHref = (elm.attribs['src'] = Uri.file(
					path.join(path.dirname(templateDocument.fileName), elm.attribs['href'])
				)
					.with({ scheme: 'vscode-resource' })
					.toString());
